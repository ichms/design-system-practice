...
@each $name, $theme in $zones {
  .cds--#{$name} {   // .cds--white, .cds--g10, .cds--g90, .cds--g100
...
```html
<div class="cds--white">   ← 이 안은 White 테마
  <Button>Light Button</Button>
</div>

<div class="cds--g90">    ← 이 안은 G90 다크 테마
...
$easings: (
  standard: (
    productive:  cubic-bezier(0.2, 0, 0.38, 0.9),   // 업무 도구 → 빠르고 효율적
    expressive:  cubic-bezier(0.4, 0.14, 0.3, 1),   // 감성 UI  → 부드럽고 풍부한 느낌
  ),
  entrance: (
